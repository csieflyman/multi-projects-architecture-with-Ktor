...
class DynamicQuery(
    ...
    companion object {
            ...
            map[QUERY_COUNT]?.toBoolean()?.also { count ->
                if (count) {
                    ...
                    if (invalidKeywords.isNotEmpty())
                        throw RequestException(ResponseCode.REQUEST_BAD_QUERY, "$QUERY_COUNT and $invalidKeywords are mutually exclusive")
                }
            },
        ...
        private fun parseOffsetLimit(offset: Long, limit: Int): OffsetLimit = try {
            if (offset < 0)
                throw RequestException(ResponseCode.REQUEST_BAD_QUERY, "$QUERY_OFFSET must be >= 0")
            if (limit < 1 || limit > MAX_LIMIT)
                throw RequestException(ResponseCode.REQUEST_BAD_QUERY, "$QUERY_LIMIT must be 1 ~ $MAX_LIMIT")

            OffsetLimit(offset, limit, false)
        } catch (e: NumberFormatException) {
            throw RequestException(ResponseCode.REQUEST_BAD_QUERY, "$QUERY_OFFSET/$QUERY_LIMIT must be integer", e)
        }

        private fun parsePaging(pageIndex: Long, itemsPerPage: Int): OffsetLimit = try {
            if (pageIndex < 1)
                throw RequestException(ResponseCode.REQUEST_BAD_QUERY, "$QUERY_PAGE_INDEX must be >= 1")
            if (itemsPerPage < 1 || itemsPerPage > MAX_LIMIT)
                throw RequestException(ResponseCode.REQUEST_BAD_QUERY, "$QUERY_ITEMS_PER_PAGE must be 1 ~ $MAX_LIMIT")

            val offset = (pageIndex - 1) * itemsPerPage
            OffsetLimit(offset, itemsPerPage, true)
        } catch (e: NumberFormatException) {
            throw RequestException(ResponseCode.REQUEST_BAD_QUERY, "$QUERY_PAGE_INDEX/$QUERY_ITEMS_PER_PAGE must be integer", e)
        }

        private fun validateOffsetLimit(offset: Long?, limit: Int?, pageIndex: Long?, itemsPerPage: Int?) {
            if ((offset != null).xor(limit != null))
                throw RequestException(ResponseCode.REQUEST_BAD_QUERY, "$QUERY_OFFSET and $QUERY_LIMIT are mutually necessary")

            if ((pageIndex != null).xor(itemsPerPage != null))
                throw RequestException(ResponseCode.REQUEST_BAD_QUERY, "$QUERY_PAGE_INDEX and $QUERY_ITEMS_PER_PAGE are mutually necessary")
    ...
    enum class PredicateOperator(
        ...
        companion object {
            ...
            fun queryStringValueOf(expr: String): PredicateOperator = try {
                ...
                throw RequestException(ResponseCode.REQUEST_BAD_QUERY, "invalid query predicate operator: $expr")
            }
        }
    }
    ...
    sealed class Predicate {
        ...
        fun getFields(): List<String> {
            return when (this) {
                ...
                is Junction -> {
                    children.flatMap { it.getFields() }.toList()
                }
            }
        }
    }

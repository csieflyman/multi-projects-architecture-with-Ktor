...
object ConfigUtils {
    ...
    fun registerMyConfigType() {
        registerCustomType(MyConfigReader)
    }
    ...
    fun require(value: Boolean, lazyMessage: () -> Any) {
        try {
            ...
            throw InternalServerErrorException(ResponseCode.SERVER_CONFIG_ERROR, e.message)
        }
    }
}

inline fun <reified T> Config.myExtract(path: String): T {
    ConfigUtils.registerMyConfigType()
    return extract(path)
}

private val MyConfigReader = object : CustomType {

    override fun parse(clazz: ClassContainer, config: Config, name: String): Any? {
        val myConfig = extractWithParameters(clazz, config, name) as MyConfig
        myConfig.validate()
        return myConfig
    }

    override fun testParse(clazz: ClassContainer): Boolean {
        return clazz.mapperClass.isSubclassOf(MyConfig::class)
    }

    override fun testToConfig(obj: Any): Boolean {
        throw InternalServerErrorException(ResponseCode.UNSUPPORTED_OPERATION_ERROR)
    }

...
interface ReportLocation {
    ...
    private fun validateQueryParameters() {
        if (reportIds.isBlank())
            throw RequestException(ResponseCode.REQUEST_BAD_QUERY, "reportIds can't be blank")
    ...
    fun toReportQueryParameters(allReports: List<Report>, call: ApplicationCall): ReportQueryParameters {
        ...
        val reports = reportIds.split(",").map { reportId ->
            allReports.find { it.id == reportId }
                ?: throw RequestException(ResponseCode.REQUEST_BAD_QUERY, "invalid reportId: $reportId")
        }
        ...
        val zoneId = try {
            ...
            throw RequestException(ResponseCode.REQUEST_BAD_QUERY, "invalid zone: $zone")
        }

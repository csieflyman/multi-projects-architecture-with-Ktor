...
class RouteApiOperation(
    ...
    private fun bindPath(routePath: String, method: HttpMethod, locationClass: KClass<*>? = null) {
        ...
        openAPIObject.addPath(path, method.value.lowercase(), operationObject)
    }
    ...
    private fun bindAuth(routeAuths: List<PrincipalAuth>?) {
        ...
        operationObject.description += "Auth => (${routeAuths?.joinToString(" or ") ?: "Public"})"
        if (routeAuths != null) {
    ...
    private fun setClientVersionHeader(routeAuths: List<PrincipalAuth>) {
        if (routeAuths.flatMap { it.allowSources }.any { it.checkClientVersion }) {

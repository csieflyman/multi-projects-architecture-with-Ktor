...
class NotificationPlugin(configuration: Configuration) {
    ...
    companion object Plugin : BaseApplicationPlugin<Application, Configuration, NotificationPlugin> {

        override val key = AttributeKey<NotificationPlugin>("Notification")

        private val logger = KotlinLogging.logger {}

        override fun install(pipeline: Application, configure: Configuration.() -> Unit): NotificationPlugin {
            val configuration = Configuration().apply(configure)
            val plugin = NotificationPlugin(configuration)
